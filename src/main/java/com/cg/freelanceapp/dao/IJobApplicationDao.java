...
//This is the DAO Interface for JobApplication module.
@Repository
public interface IJobApplicationDao extends JpaRepository<JobApplication, Long> {
	
	@Query("select new com.cg.freelanceapp.dto.JobApplicationsListDTO(j.id, j.job.id, j.job.jobTitle, j.coverLetter, j.freelancer.id, CONCAT(j.freelancer.firstName,' ', j.freelancer.lastName), j.freelancer.userName)  from JobApplication j where j.job.id=:jobId order by j.id")
	...
	@Query("select new com.cg.freelanceapp.dto.JobApplicationsListDTO(j.id, j.job.id, j.job.jobTitle, j.coverLetter, f.id, CONCAT(f.firstName,' ', f.lastName), f.userName)  from JobApplication j, Freelancer f  where j.freelancer = f and f.id=:frId and j.job.id=:jobId")
